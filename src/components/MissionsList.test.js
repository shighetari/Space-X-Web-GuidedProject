...
test("re-renders correctly with list of missions", () => {
  ...
  const missions = getAllByTestId(/mission/i); // getAll* returns an array if it finds multiple elements
  // assert
  expect(missions).toHaveLength(3);
});
test("displays an error message if an error is passed in", () => {
  //render the component with no error (don't forget to pass the missions prop)
